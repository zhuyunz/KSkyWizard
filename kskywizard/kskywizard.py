...
def parser_init():
    ...
    parser = argparse.ArgumentParser(description="KSkyWizard")
    parser.add_argument(
        '--config_path',
        action='store_true', 
        help='Return the full path of the configuration file.',
        default=False,
        )
    return parser
...
class KCWIViewerApp:
    ...
    def update_index2(self, event):
        ...
        """
        try:
            # self.index2 = int(self.index2_entry.get(), sci_index = False) #get the sky frame number
            self.get_file_prefix_index(self.index2_entry.get(), sci_index = False)
        except ValueError:
            pass #no sky frame
        self.update_index_entries()
...
def main():
    ...
    if args.config_path:
        print(pkg_resources.resource_filename(__name__, 'processing.cfg'))
        return
